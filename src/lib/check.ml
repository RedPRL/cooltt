...
struct
  ...
  let get_entry env n = List.nth env n
...
let rec check ~env ~size ~term ~tp =
  ...
  | Syn.Let (def, body) ->
    ...
    check ~env:(Env.add_term ~term:def_val ~tp:def_tp env) ~size:(size + 1) ~term:body ~tp
  | Nat ->
    begin
      match tp with
      | D.Uni _ -> ()
  ...
  | Pi (l, r) | Sg (l, r) ->
    begin
      ...
      | D.Uni _ ->
        ...
        check ~env:(Env.add_term ~term:var ~tp:l_sem env) ~size ~term:r ~tp
      | t -> tp_error (Expecting_universe t)
    end
  | Lam body ->
    begin
      ...
      | D.Pi (arg_tp, clos) ->
        ...
        check ~env:(Env.add_term ~term:var ~tp:arg_tp env) ~size:(size + 1) ~term:body ~tp:dest_tp;
...
and synth ~env ~size ~term =
  ...
  | NRec (mot, zero, suc, n) ->
    ...
    check_tp ~env:(Env.add_term ~term:var ~tp:Nat env) ~size:(size + 1) ~term:mot;
  ...
  | J (mot, refl, eq) ->
    ...
    begin
      ...
      | D.Id (tp', left, right) ->
        ...
        check ~env:(Env.add_term ~term:refl_var ~tp:tp' env) ~size:(size + 1) ~term:refl ~tp:refl_tp;
...
and check_tp ~env ~size ~term =
  ...
  | Pi (l, r) | Sg (l, r) ->
    ...
    let l_sem = Nbe.eval l (Env.to_sem_env env) in
    let var = D.mk_var l_sem size in
    check_tp ~env:(Env.add_term ~term:var ~tp:l_sem env) ~size:(size + 1) ~term:r
  | Let (def, body) ->
    let def_tp = synth ~env ~size ~term:def in
    let def_val = Nbe.eval def (Env.to_sem_env env) in
    check_tp ~env:(Env.add_term ~term:def_val ~tp:def_tp env) ~size:(size + 1) ~term:body
  | Id (tp, l, r) ->
