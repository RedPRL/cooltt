...
(** Returns whether the environment is consistent. *)
val is_consistent : env -> [`Consistent | `Inconsistent]
...
(** Assumes the truth of a cofibration; if it can be decomposed eagerly (conjunction of equations),
    ...
    sequents. The function `is_consistent` will consider cofibrations on deck. *)
